...
exports.exchangeAuthCodeForTokenAndCheckMembership = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    ...
    try {
      ...
      const requiredScope = "https://www.googleapis.com/auth/youtube.channel-memberships.creator";

      if (!scopesGranted.includes(requiredScope)) {
        return res.status(403).send({ error: "Required scope not granted." });
      }
      ...
      const isSubscribed = subscriptions.some(
        (sub) => sub.snippet.resourceId.channelId === "UCUP5RcljxXkKm3agi9WNrDA"
      );

      if (isSubscribed) {
        return res.status(200).send({ isMember: true });
      } else {
        return res.status(403).send({ message: "Not a member of the channel." });
      }
    } catch (error) {
